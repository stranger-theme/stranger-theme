...
/**
 * Build script for Stranger Theme - Vim/Neovim
 * Generates .vim color scheme files for all theme variants
 */
...
const COLORS_DIR = path.join(__dirname, '..', 'colors');

/**
 * Convert hex to Vim GUI color format
 */
function toVimColor(hex) {
  return hex;
}

/**
 * Generate Vim color scheme file content
 */
function generateVimTheme(themeId, palette) {
  const c = getDerivedColors(palette.base);
  const themeName = 'stranger_' + themeId.replace(/-/g, '_');
  const displayName = palette.displayName;
  
  return `" =============================================================================
" Name:        Stranger Theme: ${displayName}
...
let s:bg        = ['${c.BG}', 235]
let s:fg        = ['${c.FG}', 253]
let s:selection = ['${c.SELECTION}', 239]
let s:comment   = ['${c.COMMENT}', 244]
let s:cyan      = ['${c.CYAN}', 117]
let s:green     = ['${c.GREEN}', 114]
let s:orange    = ['${c.ORANGE}', 215]
let s:pink      = ['${c.PINK}', 212]
let s:purple    = ['${c.PURPLE}', 141]
let s:red       = ['${c.RED}', 203]
let s:yellow    = ['${c.YELLOW}', 228]

let s:bglight   = ['${c.BGLight}', 236]
let s:bglighter = ['${c.BGLighter}', 237]
 ...
 */
function build() {
  console.log('Building Stranger Theme for Vim/Neovim...\n');
  ...
  for (const [themeId, palette] of Object.entries(THEME_PALETTES)) {
    ...
    const vimTheme = generateVimTheme(themeId, palette);
    const themeName = 'stranger_' + themeId.replace(/-/g, '_');
    const outputFile = path.join(COLORS_DIR, `${themeName}.vim`);
    
    fs.writeFileSync(outputFile, vimTheme);
